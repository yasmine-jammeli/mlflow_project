...
# Custom CSS to hide the status widget
hide_running_status = """
    <style>
...
st.markdown(hide_running_status, unsafe_allow_html=True)
...
# Plotting Function for EEG
def plot_eeg_data(raw):
    fig, ax = plt.subplots()
    raw.plot(scalings='auto', show=False)  # Remove ax=ax here
    st.pyplot(fig)
  

# Heatmap for extracted CSV features
def plot_csv_features(df):
    fig, ax = plt.subplots(figsize=(12, 8))
    sns.heatmap(df.corr(), annot=True, fmt='.2f', cmap='coolwarm', ax=ax)
    st.pyplot(fig)

# Histogram of Signal Amplitudes
def plot_histogram(raw):
    data = raw.get_data()
    fig, ax = plt.subplots()
    ax.hist(data.flatten(), bins=50, color='skyblue', edgecolor='black')
    ax.set_title('Histogram of EEG Signal Amplitudes')
    ax.set_xlabel('Amplitude (µV)')
    ax.set_ylabel('Frequency')
    st.pyplot(fig)

# Power Spectral Density of EEG Channels
def plot_psd(raw):
    fig, ax = plt.subplots()
    raw.plot_psd(fmax=50, ax=ax)
    st.pyplot(fig)
...
if uploaded_file is not None:
    ...
    if st.session_state.processed_file_path is None or st.session_state.processed_file_path != uploaded_file.name:
        with st.spinner('Processing the EEG file...'):
            # Save the uploaded file temporarily
            with open(edf_path, 'wb') as f:
                f.write(uploaded_file.getbuffer())

            # Process the EEG file and extract features
            ...
            # Store processed file path in session state to prevent reprocessing
            st.session_state.processed_file_path = uploaded_file.name
    else:
    ...
    st.download_button(
        ...
        data=open(os.path.join(edf_dir, f'cnn_features_{os.path.splitext(uploaded_file.name)[0]}.csv'), 'rb').read(),
...
if uploaded_csv is not None:
    with st.spinner('Predicting using the processed features...'):
        ...
        # Plot CSV feature heatmap
        df = pd.read_csv(csv_path)
        st.subheader('Feature Correlation Heatmap')
        plot_csv_features(df)

        extracted_features = extract_features(csv_path)
        extracted_features_df = pd.DataFrame([extracted_features])

        # Load pre-trained model
        model_path = 'C:/Users/pc/Desktop/Nesrin_master/simple_interface/app_final/model.pkl'  # Update the path as needed
        ...
        if y_pred[0] == 0:
            st.success('Your file does not contain seizure data.')
        else:
            st.error(f'Predicted Label: {y_pred[0]}')